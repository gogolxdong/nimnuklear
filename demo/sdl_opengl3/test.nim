...
proc render() = 
  while running :
    ...
    if begin(ctx, "Garden".cstring,
      ...
      var current {.global.}: cstring = cast[cstring](alloc0(2))
      layout_row_dynamic(ctx, 35, 1)
      copyMem(buffercs, current, current.len)
      buffer[current.len + 1] = '\0'
      len = current.len.cint
      if len != 0 : echo len
      discard edit_string(ctx, EDIT_BOX.flags, buffercs, addr(len), 255, filter_default)
      buffer[len] = '\0'
      copyMem(current, buffercs, buffercs.len)
      if buffercs != "" : echo buffercs
